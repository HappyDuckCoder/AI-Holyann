...
function isMaintenanceError(error: Error): boolean {
  const msg = error.message || '';
  return (
    msg.includes('bảo trì') ||
    msg.includes('Server đang bảo trì') ||
    /fetch failed|econnrefused|enotfound|network|timeout|connection/i.test(msg)
  );
}
...
export default function GlobalError({ error, reset }: ErrorPageProps) {
  ...
  const isMaintenance = isMaintenanceError(error);
    ...
    <div className="min-h-screen flex items-center justify-center bg-background text-foreground p-6">
      ...
      >
        ...
        <h1 className="text-2xl sm:text-3xl font-heading font-bold tracking-tight text-foreground">
          {isMaintenance ? 'Hệ thống đang bảo trì' : 'Đã xảy ra lỗi'}
        </h1>
        ...
        {process.env.NODE_ENV === 'development' && error?.message && (
          <p className="mt-3 text-sm text-muted-foreground/80 font-mono break-all px-2">
            {error.message}
          </p>
        )}
