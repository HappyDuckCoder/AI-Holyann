/**
 * GET  /api/student/university-recommendation — Lấy bản ghi gợi ý đã lưu gần nhất.
...
function buildFeature2FromTests(mbti: {
  ...
  score_realistic: number | null;
  score_investigative: number | null;
  score_artistic: number | null;
  score_social: number | null;
  score_enterprising: number | null;
  score_conventional: number | null;
  top_3_types: unknown;
} | null) {
  const assessment: Record<string, unknown> = {};

  if (mbti && mbti.result_type) {
    const e = (mbti.score_e ?? 0) / 100;
    const i = (mbti.score_i ?? 0) / 100;
    assessment.mbti = {
  ...
  if (riasec) {
    ...
    assessment.riasec = {
      code: riasec.result_code ?? top3.map(([c]) => c).join(''),
      scores,
      top3,
    };
  } else {
    assessment.riasec = {
      code: 'RIA',
      scores: { Realistic: 80, Investigative: 100, Artistic: 60, Social: 40, Enterprising: 30, Conventional: 50 },
      top3: [['Investigative', 100], ['Realistic', 80], ['Artistic', 60]],
    };
  }
...
export async function GET() {
  try {
    ...
    const latest = await prisma.student_university_recommendations.findFirst({
      where: { student_id: studentId },
      orderBy: { created_at: 'desc' },
      select: { summary: true, roadmap: true, universities: true, created_at: true },
    });

    if (!latest) {
      return NextResponse.json({ success: true, saved: null }, { status: 200 });
    }
    ...
    return NextResponse.json({
      ...
      saved: {
        ...
        roadmap: latest.roadmap as object,
        universities: latest.universities as object | null,
        created_at: latest.created_at?.toISOString() ?? null,
      },
    }, { status: 200 });
  } catch (error) {
    console.error('❌ [student/university-recommendation GET] Error:', error);
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Không thể tải gợi ý đã lưu' },
      { status: 500 }
    );
  }
}
...
export async function POST() {
  try {
    ...
    const result = await callUniversityRecommendation({
      feature1_output,
      feature2_output,
      top_n: 50,
      min_match_score: 30,
    ...
    if (result && typeof result === 'object' && (result as { success?: boolean }).success) {
      const payload = result as { summary?: unknown; roadmap?: unknown; universities?: unknown };
      try {
        await prisma.student_university_recommendations.create({
          data: {
            ...
            universities: (payload.universities && typeof payload.universities === 'object') ? payload.universities : undefined,
          },
        });
      } catch (dbErr) {
        console.error('❌ [student/university-recommendation] Lưu DB thất bại:', dbErr);
        // Vẫn trả về result cho user, chỉ log lỗi lưu
      }
    }

    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('❌ [student/university-recommendation] Error:', error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Không thể lấy gợi ý trường',
      },
      { status: 500 }
    );
  }
