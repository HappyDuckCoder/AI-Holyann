...
export async function GET(
  ...
  try {
    ...
    let used = 0;
    try {
      used = await prisma.school_wishlist.count({ where: { student_id: studentId } });
    } catch (countErr) {
      console.error("[school-wishlist/limits] count failed", countErr);
    }
    ...
    console.error("[school-wishlist/limits]", e);
