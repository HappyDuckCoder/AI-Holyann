...
export default function ImprovePage() {
  ...
  const [essayList, setEssayList] = useState<
    { id: string; title: string | null; content: string; updated_at: string }[]
  >([]);
  ...
  const [essayAnalysis, setEssayAnalysis] = useState<Record<
    string,
    unknown
  > | null>(null);
  const [essayEnhance, setEssayEnhance] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const [essayAnalysisRating, setEssayAnalysisRating] = useState<number | null>(
    null,
  );
  const [essayEnhanceRating, setEssayEnhanceRating] = useState<number | null>(
    null,
  );

  // CV tab: improve results
  const [cvAnalysis, setCvAnalysis] = useState<Record<string, unknown> | null>(
    null,
  );
  const [cvEnhance, setCvEnhance] = useState<Record<string, unknown> | null>(
    null,
  );
    ...
    const load = async () => {
      ...
      try {
        ...
        if (docsRes.ok) {
          ...
          setCvList(
            docs.filter(
              (d) => d.type === "transcript" || d.name.startsWith("cv_"),
            ),
          );
        }
        if (checklistRes.ok) {
          ...
          const cvTask = tasks.find(
            (t: { title: string; link_to?: string }) =>
              t.title &&
              (t.title.toLowerCase().includes("cv") ||
    ...
    const load = async () => {
      try {
        ...
        if (
          ...
          return;
        const jobs: ImproveJob[] = data.jobs
          .map(
            (j: {
              id: string;
              job_type: string;
        ...
        if (jobs.some((j) => j.job_type === "cv_analysis"))
          setCvAnalysisLoading(true);
        if (jobs.some((j) => j.job_type === "cv_enhance"))
    ...
    const load = async () => {
      try {
        const [detailRes, resultsRes] = await Promise.all([
          ...
          fetch(
            `/api/student/improve/essay-results?essay_id=${encodeURIComponent(currentEssayId)}`,
          ),
        ]);
        if (detailRes.ok) {
  ...
  // Load CV results when opening CV or Overview tab (for dashboard cards)
  useEffect(() => {
    if (!studentId || (activeTab !== "cv" && activeTab !== "overview")) return;
    const load = async () => {
      try {
  ...
  const validateCV = (): boolean => {
    if (!cvList.length) {
      toast.error("Chưa có CV", {
        description:
          "Vui lòng tải lên ít nhất một file CV trước khi phân tích hoặc nâng cấp.",
      });
      return false;
    }
    return true;
  ...
  const pollJobResult = async (
    jobId: string,
    _kind: "analysis" | "enhance",
  ): Promise<Record<string, unknown>> => {
    const start = Date.now();
    while (Date.now() - start < POLL_MAX_WAIT_MS) {
      const res = await fetch(
        `/api/module4/profile-improver/result/${encodeURIComponent(jobId)}`,
      );
      ...
      if (data.status === "missing")
        throw new Error("Không tìm thấy kết quả. Vui lòng thử lại.");
  ...
  useEffect(() => {
    ...
    const run = async () => {
      for (const job of inProgressJobs) {
        try {
          const res = await fetch(
            `/api/module4/profile-improver/result/${encodeURIComponent(job.external_job_id)}`,
          );
          const data = (await res.json()) as {
            status: string;
            result?: Record<string, unknown>;
          ...
          await fetch("/api/student/improve/complete-job", {
            ...
            body: JSON.stringify({
              ...
              error_message:
                data.status === "error" ? data.error || "Lỗi" : undefined,
            }),
          });
  ...
  const handleEssayAnalysis = async () => {
    ...
    let delegatedToBackground = false;
    setEssayAnalysisLoading(true);
    toast.info("Đang phân tích essay... Có thể mất 1–2 phút.", {
      duration: 5000,
    });
    try {
      ...
      const data = (await res.json()) as { job_id?: string } & Record<
        string,
        unknown
      >;
      if (res.status === 202 && data.job_id) {
        try {
          const reg = await fetch("/api/student/improve/register-job", {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              job_type: "essay_analysis",
              external_job_id: data.job_id,
              essay_id: currentEssayId ?? undefined,
            }),
          });
          const regData = await reg.json();
          if (regData.success && regData.id) {
  ...
  const handleEssayEnhance = async () => {
    ...
    let delegatedToBackground = false;
    setEssayEnhanceLoading(true);
    toast.info("Đang cải thiện essay... Có thể mất 1–2 phút.", {
      duration: 5000,
    });
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(
          (err as { error?: string }).error || "Enhance thất bại",
        );
      }
      const data = (await res.json()) as { job_id?: string } & Record<
        string,
      ...
      if (currentEssayId) {
        try {
          await fetch("/api/student/improve/essay-results", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ essay_id: currentEssayId, enhance: data }),
          });
        } catch (_) {}
      }
      toast.success("Cải thiện essay thành công");
  ...
  const handleCvEnhance = async () => {
    ...
    let delegatedToBackground = false;
    setCvEnhanceLoading(true);
    toast.info("Đang tạo đề xuất cải thiện CV... Có thể mất 1–2 phút.", {
      duration: 5000,
    });
    try {
  ...
  const saveEssayRating = async (
    kind: "analysis" | "enhance",
    value: number,
  ) => {
  ...
  const handleSaveEssay = async () => {
    ...
    try {
      if (currentEssayId) {
        ...
        if (!res.ok)
          throw new Error((await res.json()).error || "Tạo bài luận thất bại");
        const json = await res.json();
  ...
  const cvScore =
    cvAnalysis && typeof cvAnalysis === "object"
      ? (cvAnalysis.overall as { score?: number })?.score ??
        (cvAnalysis as { overall_score?: number }).overall_score ??
        null
      : null;
  const essayScore =
    essayAnalysis && typeof essayAnalysis === "object"
      ? (essayAnalysis.overall as { score?: number })?.score ??
        (essayAnalysis as { overall_score?: number }).overall_score ??
        (essayAnalysis.analysis as { overall?: { score?: number } })?.overall?.score ??
        null
      : null;

  function scoreToStatus(s: number | null): SubjectStatus {
    if (s == null) return "no_data";
    if (s >= 75) return "strong";
    if (s >= 50) return "on_track";
    return "needs_work";
  }
                ...
                <div className="space-y-10">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <ImproveSubjectCard
                      ...
                      insight={
                        cvAnalysis && typeof cvAnalysis === "object"
                          ? firstWords(safeText((cvAnalysis as { summary?: unknown }).summary), 12)
                          : undefined
                      }
                      ...
                      enhanceLoading={cvEnhanceLoading}
                    ...
                    <ImproveSubjectCard
                      ...
                      insight={
                        essayAnalysis && typeof essayAnalysis === "object"
                          ? firstWords(
                              safeText((essayAnalysis as { summary?: unknown }).summary) ||
                                safeText((essayAnalysis as { feedback?: unknown }).feedback),
                              12
                            )
                          : undefined
                      }
                      ...
                      onAnalyze={() => validateEssay() && handleEssayAnalysis()}
                      onEnhance={() => validateEssay() && handleEssayEnhance()}
                      analyzeLoading={essayAnalysisLoading}
                      enhanceLoading={essayEnhanceLoading}
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mb-6">
                    Cùng một bộ file CV với trang Checklist. Tải lên tại đây sẽ
                    tự động cập nhật Checklist.
                  </p>
                  ...
                  {cvList.length === 0 ? (
                    <div className="relative border-2 border-dashed border-border rounded-2xl p-8 text-center hover:border-primary/50 hover:bg-muted/30 transition-colors">
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground">
                          Tối đa 5MB. Sẽ đồng bộ với Checklist.
                        </p>
                      </label>
                    </div>
                  ) : (
                    <div className="rounded-2xl border border-border/60 bg-muted/20 p-6 text-center">
                      <p className="text-sm font-medium text-foreground">
                        Bạn đã tải CV lên rồi. Không thể tải thêm.
                      </p>
                      <p className="text-xs text-muted-foreground mt-2">
                        CV hiển thị ở trên có thể dùng để phân tích hoặc nâng
                        cấp.
                      </p>
                    </div>
                  )}
                ...
                <div className="flex flex-wrap items-center gap-4 pt-6 border-t border-border/60">
                  ...
                  >
                    {cvTextLoading || cvAnalysisLoading ? (
                ...
                {cvAnalysis &&
                  ...
                  (() => {
                    const overall = (
                      cvAnalysis as { overall?: Record<string, unknown> }
                    ).overall;
                    const sections = (
                      cvAnalysis as {
                        ...
                        content: (
                          <div className="space-y-4">
                            {overall && (
                              <>
                                ...
                                {(safeText(overall.feedback) ||
                                  safeText(overall.summary) ||
                                  safeText(overall.personal_fit_feedback)) && (
                                  <CollapsibleSection
                                    title="Nhận xét & tóm tắt"
                                    summary="Bấm để xem"
                                    accent="violet"
                                  >
                                    {safeText(overall.feedback) && (
                                      <FormattedText
                                        text={safeText(overall.feedback)}
                                      />
                                    )}
                                    {safeText(
                                      overall.personal_fit_feedback,
                                    ) && (
                                      <p className="text-xs text-muted-foreground mt-2">
                                        Phù hợp:{" "}
                                        {safeText(
                            ...
                            {sections && Object.keys(sections).length > 0 && (
                              <CollapsibleSection
                                title="Phân tích từng phần"
                                summary={`${Object.keys(sections).length} phần · bảng điểm`}
                                accent="emerald"
                              >
                                  ...
                                  <table className="w-full text-sm text-left">
                                    <thead>
                                      <tr className="border-b border-border/60">
                                        ...
                                        <th className="py-2 text-emerald-600 dark:text-emerald-400 font-medium">
                                          Điểm
                                        </th>
                                      </tr>
                                    </thead>
                                  ...
                                  {Object.entries(sections).map(
                                    ([key, section]) => {
                                      ...
                                      const strengths = Array.isArray(
                                        ...
                                        ? (section.strengths as unknown[])
                                        : [];
                                      const weaknesses = Array.isArray(
                                        section?.weaknesses,
                                      )
                                        ? (section.weaknesses as unknown[])
                                        : [];
                                      const suggestions = Array.isArray(
                                        section?.suggestions,
                                      )
                                        ? (section.suggestions as unknown[])
                                        : [];
                                      return (
                                        <details
                                          key={key}
                                          className="group rounded-lg border border-border/40 bg-background/60 overflow-hidden mt-2"
                                        >
                            ...
                            {overall &&
                            ((Array.isArray(
                              overall.quantification_suggestions,
                            ) &&
                              overall.quantification_suggestions.length > 0) ||
                              (Array.isArray(overall.grammar_suggestions) &&
                                overall.grammar_suggestions.length > 0) ||
                              (Array.isArray(overall.priority_improvements) &&
                                overall.priority_improvements.length > 0)) ? (
                              <CollapsibleSection
                                title="Gợi ý cải thiện"
                                summary="Định lượng, ngữ pháp, ưu tiên"
                                accent="amber"
                              >
                                {Array.isArray(
                                  overall.quantification_suggestions,
                                  ...
                                  (
                                    overall.quantification_suggestions as unknown[]
                                  ).length > 0 && (
                                    <div className="mb-3">
                                ...
                                {Array.isArray(overall.grammar_suggestions) &&
                                  (overall.grammar_suggestions as unknown[])
                                    .length > 0 && (
                                    <div className="mb-3">
                                      <p className="text-xs font-medium text-amber-700 dark:text-amber-300 mb-1">
                                        Ngữ pháp
                                ...
                                {Array.isArray(overall.priority_improvements) &&
                                  (overall.priority_improvements as unknown[])
                                    .length > 0 && (
                                    <div>
                                      <p className="text-xs font-medium text-amber-700 dark:text-amber-300 mb-1">
                                        Ưu tiên cải thiện
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
                                  ...
                                  <p className="text-lg font-semibold text-primary">
                                    {String(overall.overall_score ?? "—")}/10
                                  </p>
                                </div>
                                {overall.personal_fit_score != null && (
                                  <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
                          ...
                          <div className="pt-4 border-t border-border/60 flex items-center gap-2">
                            ...
                            {[1, 2, 3, 4, 5].map((n) => (
                              <button
                                key={n}
                                type="button"
                                onClick={() => saveCvRating("analysis", n)}
                                className="p-0.5 rounded hover:bg-muted/50"
                              >
                                <Star
                                  className={`h-5 w-5 ${cvAnalysisRating != null && n <= cvAnalysisRating ? "fill-primary text-primary" : "text-muted-foreground"}`}
                                />
                              </button>
                            ))}
                          </div>
                ...
                {cvEnhance &&
                  typeof cvEnhance === "object" &&
                  (() => {
                    const overall = (
                      cvEnhance as { overall?: Record<string, unknown> }
                    ).overall;
                              ...
                              <>
                                ...
                                {Array.isArray(
                                  ...
                                  (
                                    ...
                                    <CollapsibleSection
                                      title="Gợi ý định lượng hóa"
                                      summary={`${(overall.quantification_suggestions as unknown[]).length} mục`}
                                      accent="amber"
                                    >
                                      <ul className="list-disc list-inside text-sm space-y-0.5">
                                        {(
                                ...
                                {Array.isArray(overall.grammar_suggestions) &&
                                  (overall.grammar_suggestions as unknown[])
                                    .length > 0 && (
                                    <CollapsibleSection
                                      title="Gợi ý ngữ pháp"
                                      summary={`${(overall.grammar_suggestions as unknown[]).length} mục`}
                                      accent="violet"
                                    >
                                      <ul className="list-disc list-inside text-sm space-y-0.5">
                                        {(
                          ...
                          <div className="pt-4 border-t border-border/60 flex items-center gap-2">
                            ...
                            {[1, 2, 3, 4, 5].map((n) => (
                              <button
                                key={n}
                                type="button"
                                onClick={() => saveCvRating("enhance", n)}
                                className="p-0.5 rounded hover:bg-muted/50"
                              >
                                <Star
                                  className={`h-5 w-5 ${cvEnhanceRating != null && n <= cvEnhanceRating ? "fill-primary text-primary" : "text-muted-foreground"}`}
                                />
                              </button>
                            ))}
                          </div>
                    ...
                    <>
                      ...
                      {essayAnalysis &&
                        ...
                        (() => {
                          ...
                          const overall = analysis?.overall as
                            | Record<string, unknown>
                            | undefined;
                          const assessment = analysis?.overall_assessment as
                            | Record<string, unknown>
                            | undefined;
                                ...
                                <div className="space-y-4">
                                    ...
                                    >
                                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                                        {[
                                          ...
                                          const val =
                                            (
                                              overall[key] as Record<
                                                string,
                                                unknown
                                              >
                                            )?.score ??
                                            (
                                              overall.aspect_scores as Record<
                                                string,
                                                unknown
                                              >
                                            )?.[key];
                                          ...
                                          return (
                                            <div
                                              key={key}
                                              className="rounded-lg border border-violet-200 dark:border-violet-800 bg-violet-500/10 p-2"
                                            >
                                              <p className="text-xs text-muted-foreground">
                                                {aspectNames[key] || key}
                                              </p>
                                  ...
                                  {assessment &&
                                    (safeText(assessment.summary) ||
                                      safeText(
                                        assessment.essay_level_feedback,
                                      ) ||
                                      (
                                        assessment.strengths_from_essay as unknown[]
                                      )?.length ||
                                      (
                                        assessment.weaknesses_from_essay as unknown[]
                                      )?.length ||
                                      (
                                        assessment.priority_improvements as unknown[]
                                      )?.length) && (
                                      <CollapsibleSection
                                        title="Đánh giá toàn bài"
                                        summary="Tóm tắt, điểm mạnh/yếu"
                                        accent="emerald"
                                      >
                                        <div className="space-y-3 text-sm">
                                          ...
                                          {Array.isArray(
                                            assessment.strengths_from_essay,
                                          ) &&
                                            (
                                              assessment.strengths_from_essay as unknown[]
                                            ).length > 0 && (
                                          ...
                                          {Array.isArray(
                                            assessment.weaknesses_from_essay,
                                          ) &&
                                            (
                                              assessment.weaknesses_from_essay as unknown[]
                                            ).length > 0 && (
                                          ...
                                          {safeText(
                                            assessment.personal_fit_feedback,
                                          ) && (
                                  ...
                                  {Array.isArray(weakPoints) &&
                                    weakPoints.length > 0 && (
                                      <CollapsibleSection
                                        title="Điểm yếu theo đoạn"
                                        summary={`${weakPoints.length} đoạn`}
                                        accent="rose"
                                      >
                                          ...
                                          {weakPoints.map(
                                            (
                                              ...
                                              return (
                                                <li
                                                  key={i}
                                                  className="rounded-lg border border-rose-200 dark:border-rose-800 bg-rose-500/10 p-3"
                                                >
                                                  <p>
                                                    {paraIdx != null
                                                      ? `Đoạn ${paraIdx + 1} — `
                          ...
                          return (
                            <Card className="rounded-2xl border border-border shadow-sm overflow-hidden mt-6">
                              <CardHeader className="pb-3">
                                <div className="flex items-center justify-between gap-4">
                                  <h3 className="text-base font-semibold text-foreground">
                                    Kết quả phân tích essay
                                  </h3>
                                  <button
                                    type="button"
                                    onClick={openEssayAnalysisDetail}
                                    className="text-xs font-medium text-primary hover:underline inline-flex items-center gap-1"
                                  >
                                    Xem chi tiết{" "}
                                    <ChevronRight className="h-3.5 w-3.5" />
                                ...
                                <div className="pt-4 border-t border-border/60 flex items-center gap-2">
                                  ...
                                  {[1, 2, 3, 4, 5].map((n) => (
                                    <button
                                      ...
                                      type="button"
                                      onClick={() =>
                                        saveEssayRating("analysis", n)
                                      }
                                      className="p-0.5 rounded hover:bg-muted/50"
                                    >
                                      <Star
                                        className={`h-5 w-5 ${essayAnalysisRating != null && n <= essayAnalysisRating ? "fill-primary text-primary" : "text-muted-foreground"}`}
                                      />
                                    </button>
                                  ))}
                                </div>
                      ...
                      {essayEnhance &&
                        typeof essayEnhance === "object" &&
                        (() => {
                          const enh = essayEnhance as {
                            enhanced_essay?: string;
                          ...
                          const origWords =
                            ...
                            String(enh.original_essay || essayContent)
                              .split(/\s+/)
                              .filter(Boolean).length;
                          const enhWords =
                            stats.enhanced_words ??
                            String(enh.enhanced_essay)
                              .split(/\s+/)
                              .filter(Boolean).length;
                          const changesList = Array.isArray(enh.changes)
                            ? enh.changes
                            : Array.isArray(enh.changes_log)
                              ? enh.changes_log
                              : [];
                          const analysis = enh.analysis;
                          const afterData = analysis?.after;
                          const assessment =
                            (afterData?.overall_assessment as Record<
                              string,
                              unknown
                            >) ?? {};
                          const aspectNames: Record<string, string> = {
                          ...
                          const openEssayEnhanceDetail = () => {
                            setDetailModal({
                              ...
                              content: (
                                <div className="space-y-4">
                                  {analysis?.before != null &&
                                    analysis?.after != null && (
                                      <ModalSection
                                        title="So sánh điểm trước / sau"
                                        accent="amber"
                                      >
                                        {(() => {
                                          const beforeOverall =
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                              {aspects.map((aspect) => {
                                                ...
                                                return (
                                                  <div
                                                    key={aspect}
                                                    className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-500/10 p-2"
                                                  >
                                                    <p className="text-xs text-muted-foreground">
                                                      {aspectNames[aspect] ||
                                                        aspect}
                                  ...
                                  <CollapsibleSection
                                    title="So sánh essay (gốc vs đã cải thiện)"
                                    summary={`${origWords} → ${enhWords} từ`}
                                    accent="sky"
                                  >
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                      ...
                                      <div>
                                        ...
                                        <ScrollArea className="h-40 rounded-lg border border-emerald-200 dark:border-emerald-800 bg-emerald-500/10 p-3">
                                          <FormattedText
                                            text={safeText(enh.enhanced_essay)}
                                            className="text-sm"
                                          />
                                        </ScrollArea>
                                        <p className="text-xs text-muted-foreground mt-1">
                                  ...
                                  {changesList.length > 0 && (
                                    <CollapsibleSection
                                      title="Các thay đổi theo đoạn"
                                      summary={`${changesList.length} mục`}
                                      accent="emerald"
                                    >
                                      <div className="space-y-2">
                                        {(
                                          changesList as Record<
                                            string,
                                            unknown
                                          >[]
                                        ).map(
                                          (
                                            ...
                                            if (paraIdx != null) {
                                              return (
                                                <details
                                                  key={i}
                                                  className="group rounded-lg border border-border/40 bg-background/60 overflow-hidden"
                                                >
                                                  <summary className="p-3 cursor-pointer font-medium text-foreground list-none flex items-center gap-2">
                                                    <ChevronDown className="h-4 w-4 shrink-0 transition-transform group-open:rotate-180" />{" "}
                                                    Đoạn {Number(paraIdx) + 1}
                                                  </summary>
                                                  <div className="px-3 pb-3 pt-0 space-y-2 text-sm">
                                                    {aspects.length > 0 && (
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                                      <div>
                                                        ...
                                                        <p className="whitespace-pre-wrap rounded border p-2 bg-muted/20 text-xs max-h-24 overflow-y-auto">
                                                          {original || "—"}
                                                        </p>
                                                      </div>
                                                      <div>
                                                        ...
                                                        <p className="whitespace-pre-wrap rounded border p-2 bg-emerald-500/10 text-xs max-h-24 overflow-y-auto">
                                                          {enhanced || "—"}
                                                        </p>
                                                      </div>
                                                    </div>
                                            ...
                                            return (
                                              ...
                                              >
                                                <summary className="p-3 cursor-pointer font-medium text-foreground list-none">
                                                  Cải thiện toàn diện
                                                </summary>
                                                <div className="px-3 pb-3 pt-0 text-sm text-muted-foreground">
                                                  Đã cải thiện toàn bộ essay.
                                                </div>
                                              </details>
                                            );
                                          },
                                  ...
                                  {(safeText(assessment?.summary) ||
                                    ...
                                    <CollapsibleSection
                                      title="Đánh giá sau cải thiện"
                                      summary="Bấm để xem"
                                      accent="violet"
                                    >
                                      {safeText(assessment.summary) && (
                                        <FormattedText
                                          text={safeText(assessment.summary)}
                                        />
                                      )}
                                      {safeText(
                                        assessment.essay_level_feedback,
                          ...
                          return (
                            <Card className="rounded-2xl border border-border shadow-sm overflow-hidden mt-6">
                              <CardHeader className="pb-3">
                                <div className="flex items-center justify-between gap-4">
                                  <h3 className="text-base font-semibold text-foreground">
                                    Essay đã được cải thiện
                                  </h3>
                                  <button
                                    type="button"
                                    onClick={openEssayEnhanceDetail}
                                    className="text-xs font-medium text-primary hover:underline inline-flex items-center gap-1"
                                  >
                                    Xem chi tiết{" "}
                                    <ChevronRight className="h-3.5 w-3.5" />
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                  ...
                                  <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
                                    ...
                                    <p className="text-sm font-medium">
                                      {stats.limit_words != null &&
                                      enhWords <= stats.limit_words
                                        ? "✅ Trong giới hạn"
                                        : stats.limit_words != null
                                          ? "⚠️ Vượt"
                                          : "—"}
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    {[1, 2, 3, 4, 5].map((n) => (
                                      <button
                                        ...
                                        type="button"
                                        onClick={() =>
                                          saveEssayRating("enhance", n)
                                        }
                                        className="p-0.5 rounded hover:bg-muted/50"
                                      >
                                        <Star
                                          className={`h-5 w-5 ${essayEnhanceRating != null && n <= essayEnhanceRating ? "fill-primary text-primary" : "text-muted-foreground"}`}
                                        />
                                      </button>
                                    ))}
                                  </div>
