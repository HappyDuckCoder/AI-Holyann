...
export const generateSwotAnalysis = async (
  ...
  if (!apiKey) {
    throw new Error(
      "API Key is missing. Please provide a valid NEXT_PUBLIC_API_KEY in the environment.",
    );
  }
          ...
          items: {
            ...
            properties: {
              ...
              category: {
                type: Type.STRING,
                description:
                  "Phân loại: STEM, Nghệ thuật, Kinh doanh, Nhân văn, Khoa học xã hội, v.v.",
              },
              matchReason: {
                type: Type.STRING,
                description:
                  "Lý do cụ thể tại sao ngành này phù hợp với tính cách (150-200 từ)",
              },
              careerPaths: {
                type: Type.ARRAY,
                items: { type: Type.STRING },
...
// CV Scanning with AI
export const scanCVWithAI = async (
  cvText: string,
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            id: {
              type: Type.STRING,
              description: "Generate random ID like 'STU' + 6 digits",
            },
            ...
            avatarUrl: {
              type: Type.STRING,
              description:
                "Use default: https://api.dicebear.com/9.x/notionists/svg?seed=Felix",
            },
            gpa: { type: Type.NUMBER, description: "GPA/điểm trung bình" },
            gpaScale: {
              type: Type.NUMBER,
              description: "Thang điểm (thường là 4.0 hoặc 10.0)",
            },
            englishLevel: {
              type: Type.STRING,
              description: "Trình độ tiếng Anh (IELTS/TOEFL)",
            },
            satScore: { type: Type.INTEGER, description: "SAT score nếu có" },
            targetMajor: {
              type: Type.STRING,
              description: "Ngành học mong muốn",
            },
            targetCountry: {
              type: Type.STRING,
              description: "Quốc gia mong muốn du học",
            },
            extracurriculars: {
              type: Type.ARRAY,
              items: {
