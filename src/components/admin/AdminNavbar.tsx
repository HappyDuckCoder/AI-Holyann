        ...
        <header className="navbar-header sticky top-0 z-50 w-full px-4 sm:px-6 md:px-8">
            <div className="container flex h-16 max-w-screen-2xl items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <Link href="/admin/dashboard" className="flex items-center gap-3 group focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/40 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 rounded-lg transition-all duration-200 ease-in-out active:scale-[0.98]">
                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary-foreground/10 border border-primary-foreground/20 transition-colors group-hover:bg-primary-foreground/15 dark:bg-white/10 dark:border-white/10 dark:group-hover:bg-white/15 md:h-10 md:w-10">
                            <Shield className="h-4 w-4 text-accent md:h-5 md:w-5" />
                        </div>
                        <div className="flex flex-col leading-none gap-0.5">
                            <span className="font-semibold tracking-[0.08em] text-primary-foreground dark:text-white">HOLYANN</span>
                            <span className="text-[10px] font-medium uppercase tracking-[0.25em] text-primary-foreground/60 md:text-xs dark:text-white/60">Quản trị viên</span>
                        </div>
                    </Link>
                </div>
                        ...
                        <div className="relative" ref={userMenuRef}>
                            ...
                            {userMenuOpen && (
                                <div className="absolute right-0 top-full z-50 mt-2 w-56 origin-top-right rounded-xl border border-border bg-popover dark:border-white/10 dark:bg-slate-900/95 dark:backdrop-blur-md p-1 shadow-lg text-foreground animate-in fade-in-0 zoom-in-95 duration-200">
                                    ...
                                    <Link href="/admin/settings" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm text-foreground hover:bg-muted dark:text-white/90 dark:hover:bg-slate-800 transition-colors">
                    ...
                    <nav className="container flex flex-col gap-1 px-4 py-3">
                        ...
                        {isAuthenticated && (
                            <div className="mt-2 border-t border-primary-foreground/20 dark:border-white/6 pt-2">
                                <Link href="/admin/settings" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 rounded-full px-4 py-3 text-sm text-primary-foreground/90 hover:bg-primary-foreground/10 dark:text-white/90 dark:hover:bg-slate-800 transition-all duration-200">
                                    <Settings className="h-4 w-4 opacity-70" /> Cài đặt
                                </Link>
