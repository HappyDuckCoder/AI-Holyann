...
export default function MentorNavbar() {
    ...
    return (
        ...
        >
            <div className="absolute inset-x-0 top-0 h-[2px] bg-linear-to-r from-transparent via-white/30 to-transparent pointer-events-none dark:via-primary/40" />
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {item.name}
                                {active && (
                                    <span className="absolute inset-x-3 -bottom-0.5 h-px bg-linear-to-r from-transparent via-white/50 to-transparent rounded-full" />
                                )}
                            </Link>
                        )
                    })}
                        ...
                        <div className="relative hidden md:block" ref={userMenuRef}>
                            ...
                            {userMenuOpen && (
                                <div className="absolute right-0 top-full z-50 mt-2 w-56 origin-top-right rounded-xl border border-border bg-card p-1 shadow-xl text-foreground animate-in fade-in-0 zoom-in-95 duration-200">
                                    ...
                                    <Link href="/mentor/profile" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm text-foreground hover:bg-muted transition-colors">
                                        <User className="h-4 w-4 text-muted-foreground" /> Hồ sơ
                                    </Link>
                    ...
                    <nav className="container flex flex-col gap-1 px-4 py-3">
                        ...
                        {isAuthenticated && (
                            <div className="mt-2 border-t border-white/10 pt-2">
                                <Link href="/mentor/profile" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 rounded-full px-4 py-3 text-sm text-white/85 hover:bg-white/10 hover:text-white transition-all duration-200">
